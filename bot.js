...
client.on('message', async message => {
    ...
    var subChannel = message.guild.channels.find(c => c.name === '✵-「accept-support');// Alpha Codes
   
    if(command == prefix + 'apply') {// Alpha Codes
        if(message.author.bot) return;
        if(message.channel.type === 'dm') return;
 
        var modRole = message.guild.roles.find(r => r.name === '• OmEGa » Support');// Alpha Codes
                    ...
                    message.channel.awaitMessages(filter, { max: 1, time: 30000, errors: ['time'] }).then(collected => {
                        ...
                        msgS.edit(':timer: | **من فضلك اكتب من اي بلد انت**').then(msgS => {
                            message.channel.awaitMessages(filter, { max: 1, time: 30000, errors: ['time'] }).then(collected => {
                                ...
                                collected.first().delete();
                                msgS.edit(':timer: | **معك خبرة بالاكواد ؟**').then(msgS => {
                                    message.channel.awaitMessages(filter, { max: 1, time: 30000, errors: ['time'] }).then(collected => {
                                        ...
                                        .addField('معك خبرة بالاكواد ؟', fa2dh, true)
                                        .setTimestamp()
                                            ...
                                            yesSend.on('collect', r => {
                                                ...
                                                subChannel.send(subMsg).then(msgS => {
                                                    ...
                                                    acceptRe.on('collect', r => {
                                                        msgS.delete();
                                                        message.author.send(`:white_check_mark: | تم قبولك كـ سبورت **${message.guild.name}**`);
                                                        message.guild.member(message.author).addRole(modRole.id);
                                                        message.guild.channels.find(r => r.name === '✵-「accept-submite').send(`:white_check_mark: | تم قبولك [ <@${message.author.id}> ]`);
                                                    }).catch();
                                                    noAcceptRe.on('collect', r => {
                                                        msgS.delete();
                                                        message.author.send(`:x: | تم رفضك بسيرفر **${message.guild.name}**`);
                                                        message.guild.channels.find(r => r.name === '✵-「accept-submite').send(`:x: | تم رفضك [ <@${message.author.id}> ]`);
                                                    }).catch();
                                                })
                                            });// Alpha Codes
