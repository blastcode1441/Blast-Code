...
Client.on('message', async message => {
  ...
  var subChannel = message.guild.channels.find(c => c.name === '✵-「accept-submite');
 
  if(command == prefix + 'apply') {
      if(message.author.bot) return;
      if(message.channel.type === 'dm') return;

      var modRole = message.guild.roles.find(r => r.name === '• OmEGa » Support');
                                  ...
                                  message.channel.awaitMessages(filter, { max: 1, time: 30000, errors: ['time'] }).then(collected => {
                                      ...
                                      message.channel.send(embedS).then(msgS => {
                                          ...
                                          yesSend.on('collect', r => {
                                              msgS.delete();
                                              message.channel.send(':white_check_mark: | تم تقديم طلبك بنجاح انتظر النتيجة في روم #✵-「accept-submite').then(msg => msg.delete(5000));
...
'
                                                  let noAccept = (reaction, user) => reaction.emoji.name === '❎' && user.id === '484725335741431818
'
